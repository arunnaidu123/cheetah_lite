...
template<typename Type>
__global__ void reduce_freq(Type * values, Type * results, unsigned int number_of_channels, unsigned int number_of_samples) {
...
// MODIFIED, not equivalent to Linus code because our data structures are different
template<typename Type>
__global__ void flagger_freq(Type * values, LocalFlagsType * global_flags, unsigned int * nr_flagged, unsigned int number_of_channels, unsigned int number_of_samples) {
    extern __shared__ Type shared[];
    ...
    // NOTE: terrible memory access pattern
    global_flags[(threadIdx.x * number_of_samples) + blockIdx.x] = local_flags[tid] | global_flags[(threadIdx.x * number_of_samples) + blockIdx.x];
}

// MODIFIED, not equivalent to Linus code because our data structures are different
template<typename Type>
__global__ void flagger_time(Type * values, LocalFlagsType * global_flags, unsigned int * nr_flagged, unsigned int number_of_samples) {
    extern __shared__ Type shared[];
    ...
    global_flags[(blockIdx.x * number_of_samples) + threadIdx.x] = local_flags[tid] | global_flags[(blockIdx.x * number_of_samples) + threadIdx.x];
}